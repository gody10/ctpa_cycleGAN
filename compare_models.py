#!/usr/bin/env python3
"""
Common Evaluation Script — Fair comparison of 3D Latent Diffusion vs CycleGAN.
...
Metrics:
    ...
    - FID  (Fréchet Inception Distance) — computed across all slices from all volumes

Usage:
    python compare_models.py \
        --diffusion_dir  ./predictions_medvae_ldm/ \
        --cyclegan_dir   ./results/cyclegan_3d_volumes/ \
...
# ============================================================================
# FID — Fréchet Inception Distance on 2D slices
# ============================================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare 3D Latent Diffusion vs CycleGAN volumes",
    ...
    parser.add_argument(
        ...
        help="Folder of 3D .nii.gz volumes from CycleGAN (inference_and_stitch.py output)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cyclegan_suffix", type=str, default="_cyclegan_pred",
        help="Suffix to strip from CycleGAN filenames to get patient ID"
    )
    parser.add_argument(
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("MODEL COMPARISON — 3D Latent Diffusion vs CycleGAN")
    logger.info("=" * 70)
    logger.info(f"Diffusion dir:  {args.diffusion_dir}")
    logger.info(f"CycleGAN dir:   {args.cyclegan_dir}")
    ...
    if args.gt_from_cyclegan:
        ...
        logger.info("Using GT volumes from CycleGAN output directory for both models.")
    elif args.gt_dir:
    ...
    # ── Summary statistics ────────────────────────────────────────────────
    summary_rows = []
    for model_name, df in [("Diffusion", diff_df), ("CycleGAN", cyc_df)]:
        if df.empty:
            continue
        row = {
            "Model": model_name,
        ...
        if gt_feats is not None:
            ...
            for row in summary_rows:
                if row["Model"] == "Diffusion":
                    row["FID"] = fid_diff
                elif row["Model"] == "CycleGAN":
                    row["FID"] = fid_cyc

            summary_df = pd.DataFrame(summary_rows)
            logger.info(f"FID — Diffusion: {fid_diff:.2f},  CycleGAN: {fid_cyc:.2f}")
        else:
