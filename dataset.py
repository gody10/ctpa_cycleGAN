"""
Dataset for CycleGAN baseline with exact data parity to the 3D Latent Diffusion model.
...
Normalization Contract (matches ctpa_medvae_latent_diffusion exactly):
    ...
    - Then rescaled to [-1, 1] for CycleGAN (Tanh output range)
...
Two dataset classes:
    ...
    - ColteaSliceDataset:    Returns individual 2D axial slices on the fly
                             (for CycleGAN training and validation).
"""
...
# ============================================================================
# Dataset 2 — 2D slice dataset (for CycleGAN training / validation)
# ============================================================================
...
class ColteaSliceDataset(Dataset):
    ...
    (1, H, W) — ready for CycleGAN's 1-channel input.

    For CycleGAN compatibility the returned dict uses keys
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Convert [0, 1] -> [-1, 1] (CycleGAN Tanh range)
...
# ============================================================================
# Legacy wrapper — kept for backward compatibility with old train.py
# ============================================================================

class ColteaPairedDataset(Dataset):
    """
    Legacy wrapper. New code should use ColteaSliceDataset or
    ColteaPairedDataset3D directly.
    """
...
def get_transforms(mode="train"):
    """Legacy transform builder — uses fixed HU windowing for parity."""
    transforms = [
    ...
    if mode == "train":
        transforms.append(
            RandSpatialCropd(
                keys=["image", "label"],
                roi_size=(TARGET_XY, TARGET_XY, MIN_DEPTH),
                random_center=True,
                random_size=False,
            )
        )
